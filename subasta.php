    ...
    <?php
      if (isset($_POST["ofertar"])) {
        ...
        $fecha_hora_actual = date("Y-m-d H:i:s");
          ...
          if($oferta == $max){
            //codigo para realizar la compra directamente;
            $res_1 = $bd->query("INSERT into oferta(oferta, estado, fecha_hora, subasta_id_subasta, comprador_idcomprador) values($oferta, 1, '$fecha_hora_actual',$id_sub_1, $id_user_1);");
            if($res_1 == false){
            ...
            $res_1 = $bd->query("INSERT into oferta(oferta, estado, fecha_hora, subasta_id_subasta, comprador_idcomprador) values($oferta, 0, '$fecha_hora_actual',$id_sub_1, $id_user_1);");
            if($res_1 == false){
              echo "<script>alert('No se ha podido ofertar');</script>";
            }else{
              $res_2_1 = $bd->query("UPDATE subasta set estado=1,comprador=$id_user_1 where id_subasta=$id_sub_1;");
                  ...
                  <?php
                      ...
                      if($res->num_rows > 0){
                        while($row = $res->fetch_assoc()){
                          ...
                          $subastador = $row["usuario_idusuario"];
                          $id_producto = $row["producto_idproducto"];
                          ...
                          $res2 = $bd->select("SELECT * from producto where idproducto=$id_producto");
                          if($res2->num_rows > 0){
                            while($row2 = $res2->fetch_assoc()){
                              $nombre_p = $row2["nombre"];
                              $imagen_p = $row2["imagen"];
                              $descripcion_p = $row2["descripcion"];
                              $id_categoria = $row2["categoria_id_categoria"];

                              //Inicia consulta de categoria del producto
                              //muestra en q categoria esta el producto subastado
                              ...
                              /*codigo q muestra en pantalla*/
                              /*la cantidad de ofertanes q ese producto*/
                              $res_count=$bd->select("SELECT count(*) as total from oferta where subasta_id_subasta=$id_sub");
                              $data=mysqli_fetch_array($res_count);
                              $count_ofert = $data['total'];

                             /*codigo realiza consulta en la tabla oferta para qu me muestre en pantalla 
                             la oferta actual q se ha echo por el producto*/

                              $res3 = $bd->select("SELECT * from oferta where subasta_id_subasta=$id_sub order by id_oferta desc limit 1");
                              if($res3->num_rows > 0){
                                while($row3 = $res3->fetch_assoc()){
                                  $id_oferta = $row3["id_oferta"];
                                  $oferta = $row3["oferta"];
                                  $ofertante_comp = $row3["comprador_idcomprador"];

                                  

                                  /*div mostraran los productos que tienen una oferta ya*/
                                  ?>
                                  <div class="col-sm-6 col-md-6">
                                      
                                      <?php
                                        // div muestrar la imagen del producto en grande
